...
#define BASE  0 
#define ITERS 1
#define DELTA 15       // Right now only set up to do one size
...
#define OMEGA_INC 0.016   // OMEGA increment for each O_ITERS
...
void SOR(vec_ptr v, int *iterations)
{
  ...
  while ((mean_change/(double)(length*length)) > (double)TOL) {
    iters++;
    mean_change = 0;
    #pragma omp parallel shared(change,data,length) private(i,j)
    #pragma omp for
